...
public class UserDao {
	public static List<User> getAllUser() throws SQLException {
		...
		try {
			...
			while (resultSet.next()) {
				...
				u.setRole_id(UserRoleDao.getRoleIdByUserId(resultSet.getInt("user_id")));
				u.setActivated(resultSet.getBoolean("activated"));
				u.setUsername(resultSet.getString("username"));
	...
	public static void deactivateUserById(int userId, boolean activated) throws SQLException {
	    try {
	        Connection connection = DatabaseConnection.getConnection();
            PreparedStatement statement = connection.prepareStatement("UPDATE users SET activated = ? WHERE user_id = ?");
            statement.setBoolean(1, activated);
	...
	public static User authenticate(String username, String password) throws SQLException {
	    ...
	    try {
	        connection = DatabaseConnection.getConnection();
	        statement = connection.prepareStatement("SELECT u.user_id, u.username, u.password, u.full_name, u.email, u.phone_number, ur.role_id, u.activated FROM users u JOIN userroles ur ON u.user_id = ur.user_id WHERE u.username = ? AND u.password = ?");
