...
public class UserRoleDao {
	...
	public static void addUserRole(UserRole pro) throws SQLException {
		try {
			...
			PreparedStatement statement = connection
					.prepareStatement("INSERT INTO userroles(user_id, role_id) VALUES(?,?)");
	...
	public static int getRoleIdByUserId(int userId) throws SQLException {
		int roleId = -1;
		try {
			Connection connection = DatabaseConnection.getConnection();
			PreparedStatement statement = connection
					.prepareStatement("SELECT role_id FROM userroles WHERE user_id = ?");
